...
from .chart_widget import ChartWidget, CharWidgetConfiguration
...
class LineChartWidget(ChartWidget):
    ...
    def __init__(self, parent_surface: pygame.Surface, name: str, rect: pygame.Rect, background_color: tuple[int, int, int] = None, border: bool = False, border_color: tuple[int, int, int] = DEFAULT_WIDGET_BORDER_COLOR, config: CharWidgetConfiguration = None, data_source: MQTTDataSource = None) -> None:
        super().__init__(parent_surface = parent_surface, name = name, rect = rect, background_color = background_color, border = border, border_color = border_color, config = config)
        self._refresh_required = True
        #self.__graph_surface = pygame.Surface((self.width - 10, self.height - 10))
        self.__graph_surface = pygame.Surface((self._chart_width, self._chart_height))
